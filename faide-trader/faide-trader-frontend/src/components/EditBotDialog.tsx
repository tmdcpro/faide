...
interface EditBotDialogProps {
  ...
  onSubmit: (data: { name: string; strategy_type: string; symbol: string; symbols: string[]; is_active: boolean }) => Promise<void>;
  onClose: () => void;
}
...
export function EditBotDialog({ bot, accountExchange, onSubmit, onClose }: EditBotDialogProps) {
  const [name, setName] = useState(bot.name);
  const [strategyType, setStrategyType] = useState(bot.strategy_type);
  const [selectedSymbols, setSelectedSymbols] = useState<string[]>(
    bot.symbols.length > 0 ? bot.symbols : [bot.symbol]
  );
